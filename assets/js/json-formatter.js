...
(function () {
  ...
  function render(text, value, opts) {
    lastText = text;
    lastValue = value;
    lastPlain = !!(opts && opts.plain);
    output.classList.remove("placeholder");
    if (!opts || !opts.keepTab) showTextTab();
    renderFoldableOutput(text, value, opts);
    renderTree(value);
  ...
  function tryAutoFix(text) {
    ...
    for (const [name, fn] of steps) {
      ...
      if (next && next !== cur) {
        cur = next;
        try {
          const value = JSON.parse(cur);
          return { value, applied: [...applied, name] };
        } catch (_) { applied.push(name); }
      }
    }
    try {
      const balanced = balanceBrackets(cur);
      if (balanced && balanced !== text) {
        const value = JSON.parse(balanced);
        return { value, applied: [...applied, "括号补全"] };
      }
    } catch (_) { /* 仍失败则返回 null */ }
    return null;
  }
  ...
  function doAutoFix(ev) {
    ...
    if (result) {
      ...
      const pretty = indent === null ? JSON.stringify(result.value) : JSON.stringify(result.value, null, indent);
      input.value = pretty; // 用户主动点的修复，允许写回左侧
      updateInputCount(); renderInputHighlight(); save();
      render(pretty, result.value);
  ...
  function renderFoldableOutput(text, value, opts) {
    ...
    output.appendChild(buildFoldNode(value, 0, indentStr, true));
  }
  ...
  function scheduleAutoFormat() {
    ...
    autoTimer = setTimeout(() => {
      ...
      try {
        ...
        render(text, value, { keepTab: true });
  ...
  function doMinify() {
    try {
      ...
      render(JSON.stringify(value), value, { plain: true });
      $("st-status").textContent = "✅ 有效 JSON";
      save();
    } catch (e) {
      renderError(e);
  ...
  function doUnescape() {
    ...
    try {
      ...
      const value = typeof once === "string" ? once : once;
      const display = typeof once === "string" ? once : JSON.stringify(once);
      // 去转义后若仍是合法 JSON 则格式化显示，否则按原串显示
      try {
        const v2 = JSON.parse(display);
        const indent = getIndent();
        const text = indent === null ? JSON.stringify(v2) : JSON.stringify(v2, null, indent);
        render(text, v2);
      } catch (_) {
        lastText = display;
        lastValue = value;
  ...
  function restoreRightView() {
    if (lastText && lastValue !== null && lastValue !== undefined) {
      output.classList.remove("placeholder");
      renderFoldableOutput(lastText, lastValue, { plain: lastPlain });
    } else if (lastText) {
      const pre = document.createElement("pre");
      pre.className = "j-plain";
